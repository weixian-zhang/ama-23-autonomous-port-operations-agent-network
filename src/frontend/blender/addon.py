...
class BlenderMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        handlers = {
            ...
            "create_20ft_container": self.create_20ft_container,
    ...
    def create_20ft_container(self, name="Container_20ft", location=None):
        """Create a detailed 20-foot shipping container with corrugation line details.

        Generates a realistic ISO 20ft container with:
        - Vertical corrugation ridges on both side walls
        - Lengthwise corrugation ridges on the roof
        - Double doors on the rear end with vertical reinforcement bars
        - Lock rod assemblies with cam handles
        - Corner castings at all 8 corners
        - Bottom side rails and cross members
        - Front wall (non-door end)
        """
        try:
            if location is None:
                location = [0, 0, 0]

            x0, y0, z0 = float(location[0]), float(location[1]), float(location[2])

            # ISO 20ft container external dimensions (meters)
            L = 6.058    # length along X
            W = 2.438    # width along Y
            H = 2.591    # height along Z

            wall_t = 0.003     # wall thickness (3mm steel)

            # Side wall corrugation (vertical ridges)
            side_corr_depth = 0.035
            side_corr_width = 0.045
            side_corr_pitch = 0.115

            # Roof corrugation (lengthwise ridges)
            roof_corr_depth = 0.025
            roof_corr_width = 0.040
            roof_corr_pitch = 0.100
            ...
            def _make_box(bname, center, dims):
                """Create a box mesh at center with given dimensions."""
                ...
                m = bpy.data.meshes.new(bname + "_mesh")
                o = bpy.data.objects.new(bname, m)
                bpy.context.collection.objects.link(o)
                m.from_pydata(verts, [], faces)
                m.update()
                objects_to_join.append(o)
                return o
            ...
            # ── Side wall base panels ──
            _make_box("cnt_left_wall",
                      (x0, y0 - W / 2 + wall_t / 2, z0 + H / 2),
                      (L, wall_t, H))
            _make_box("cnt_right_wall",
                      (x0, y0 + W / 2 - wall_t / 2, z0 + H / 2),
                      (L, wall_t, H))
            ...
            # ── Front wall (non-door end) ──
            _make_box("cnt_front_wall",
                      (x0 - L / 2 + wall_t / 2, y0, z0 + H / 2),
                      (wall_t, W, H))

            # ── Door-end top beam ──
            _make_box("cnt_door_top_beam",
                      (x0 + L / 2 - wall_t / 2, y0, z0 + H - 0.06),
                      (wall_t * 3, W, 0.12))

            # ── Side wall vertical corrugation ridges ──
            panel_h = H - rail_h - 0.06
            panel_bot = z0 + rail_h
            margin = 0.18  # margin at each end of the wall
            usable_length = L - 2 * margin
            n_side = int(usable_length / side_corr_pitch)
            sx = x0 - L / 2 + margin + side_corr_pitch / 2

            for i in range(n_side):
                rx = sx + i * side_corr_pitch
                if rx > x0 + L / 2 - margin:
                    break
                # Left wall ridge (outward −Y)
                _make_box(f"cnt_lr_{i}",
                          (rx, y0 - W / 2 - side_corr_depth / 2,
                           panel_bot + panel_h / 2),
                          (side_corr_width, side_corr_depth, panel_h))
                # Right wall ridge (outward +Y)
                _make_box(f"cnt_rr_{i}",
                          (rx, y0 + W / 2 + side_corr_depth / 2,
                           panel_bot + panel_h / 2),
                          (side_corr_width, side_corr_depth, panel_h))

            # ── Roof lengthwise corrugation ridges ──
            roof_usable = W - 0.20
            n_roof = int(roof_usable / roof_corr_pitch)
            sy = y0 - roof_usable / 2 + roof_corr_pitch / 2

            for i in range(n_roof):
                ry = sy + i * roof_corr_pitch
                if ry > y0 + roof_usable / 2:
                    break
                _make_box(f"cnt_rrf_{i}",
                          (x0, ry, z0 + H + roof_corr_depth / 2),
                          (L - 0.20, roof_corr_width, roof_corr_depth))

            # ── Bottom side rails ──
            _make_box("cnt_rail_l",
                      (x0, y0 - W / 2 + rail_t / 2, z0 + rail_h / 2),
                      (L, rail_t, rail_h))
            _make_box("cnt_rail_r",
                      (x0, y0 + W / 2 - rail_t / 2, z0 + rail_h / 2),
                      (L, rail_t, rail_h))
            ...
            for i in range(n_cross):
                cx = x0 - L / 2 + (i + 1) * L / (n_cross + 1)
                _make_box(f"cnt_xm_{i}",
                          (cx, y0, z0 - 0.006),
                          (0.012, W, 0.012))

            # ── Corner castings (8 corners) ──
            idx = 0
            for xs in [-1, 1]:
                for ys in [-1, 1]:
                    for zs in [0, 1]:
                        ccx = x0 + xs * (L / 2 - cc_size / 2)
                        ccy = y0 + ys * (W / 2 - cc_size / 2)
                        if zs == 0:
                            ccz = z0 + cc_h / 2
                        else:
                            ccz = z0 + H - cc_h / 2
                        _make_box(f"cnt_cc_{idx}",
                                  (ccx, ccy, ccz),
                                  (cc_size, cc_size, cc_h))
                        idx += 1

            # ── Door end – two door panels ──
            door_w = (W - 0.12) / 2
            door_h = H - 0.16
            door_x = x0 + L / 2 + wall_t / 2

            _make_box("cnt_door_L",
                      (door_x, y0 - door_w / 2 - 0.02,
                       z0 + door_h / 2 + 0.08),
                      (wall_t, door_w, door_h))
            _make_box("cnt_door_R",
                      (door_x, y0 + door_w / 2 + 0.02,
                       z0 + door_h / 2 + 0.08),
                      (wall_t, door_w, door_h))
            ...
            n_bars = 4
            for side_label, yc in [("L", y0 - door_w / 2 - 0.02),
                                   ("R", y0 + door_w / 2 + 0.02)]:
                for i in range(n_bars):
                    by = yc - door_w / 2 + (i + 1) * door_w / (n_bars + 1)
                    _make_box(f"cnt_db_{side_label}_{i}",
                              (door_x + wall_t + door_bar_d / 2,
                               by,
                               z0 + door_h / 2 + 0.08),
                              (door_bar_d, door_bar_w, door_h - 0.10))

            # ── Lock rods ──
            for side_label, yo in [("L", -0.07), ("R", 0.07)]:
                _make_box(f"cnt_lkr_{side_label}",
                          (door_x + wall_t + lock_rod_d / 2,
                           y0 + yo,
                           z0 + door_h / 2 + 0.08),
                          (lock_rod_d, lock_rod_w, door_h + 0.06))

            # ── Lock rod cam handles ──
            for side_label, yo in [("L", -0.07), ("R", 0.07)]:
                _make_box(f"cnt_lkh_{side_label}",
                          (door_x + wall_t + 0.04,
                           y0 + yo,
                           z0 + H * 0.55),
                          (0.06, 0.08, 0.03))
            ...
            # ── Door hinge brackets ──
            hinge_positions_z = [z0 + 0.25, z0 + H * 0.5, z0 + H - 0.25]
            for side_label, ys_sign in [("L", -1), ("R", 1)]:
                edge_y = y0 + ys_sign * (W / 2 + 0.005)
                for hi, hz in enumerate(hinge_positions_z):
                    _make_box(f"cnt_hng_{side_label}_{hi}",
                              (door_x + 0.01, edge_y,  hz),
                              (0.04, 0.06, 0.08))
            ...
            if objects_to_join:
                ...
                # Material – light grey steel matching reference image
                mat = bpy.data.materials.new(name="Container_Steel")
                mat.use_nodes = True
                ...
                for poly in container.data.polygons:
                    poly.use_smooth = True

                return {
                    "success": True,
                    "message": f"20ft container '{name}' created with "
                               f"corrugation details",
                    "name": container.name,
                    "location": [round(container.location.x, 3),
                                 round(container.location.y, 3),
                                 round(container.location.z, 3)],
                    "parts": len(objects_to_join),
                }

            return {"error": "No geometry created"}

        except Exception as e:
            print(f"Error creating 20ft container: {str(e)}")
            traceback.print_exc()
            return {"error": str(e)}
