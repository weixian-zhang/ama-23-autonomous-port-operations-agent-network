...
* use `workflow` to create infographics of agent process flow diagram, step-by-step arrows.
* create "Job Auction Cognitive Bidding" in Container-Move Workflow but do not block view of any agents.
* `Planner Workflow` workflow arrows should be faint glowing blue. Include workflow order number in `Planner Workflow`
* `Container-Move Workflow` workflow arrows should be faint glowing yellow. Include workflow order number in `Container-Move Workflow`
* create graphic for each agent to visually describe component.
* separate "planner agents" and "continer-move agents"
* include `cloud and edge agents description` as agents description. but free to simplify description if causing infographics to be distorted
* add "Supervisor Pattern" under "Planner Agent"
* add "Agent Mesh Pattern" under "Container-Move Agents"
...
cloud agents
    ...
    Container--Move Agent
        ...
        Role: The Logistics Broker.
            ...
            * syncs with MetaRealm for AGV and stackers posution and state.
...
## Planner Workflow
    1. Apex Planner Agent -> (create/edit vessel-to-berth plan) Berth Planner Agent -> Vessel Arrival Oracle Agent (predict vessel ETA) 
    2. Apex Planner Agent -> Crane Auctioneer Agent (create/edit crane-to-bay plan) ->  Crane Edge Agent (auction crane assignment jobs)
    3  Apex Planner Agent -> Yard King Agent (create/edit yard allocation plan) 

## Container-Move Workflow
    1. Crane Auctioneer Agent <-> Crane Edge Agent (auction-bid-award crane job)
    2. Crane Auctioneer Agent -> Fleet Market (request AGV to berth) 
    3. Fleet Market Agent <-> to AGV Edge Agent (auction-bid-award AGV job)
    4. Fleet Market Agent -> Yard King (get container yard position)
    5. AGV (announce arrival at yard with container) -> Fleet Market ->  Stacker Edge Agent (auction-bid-award stacker job unload container to yard)
